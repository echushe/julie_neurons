...
Sigmoid_CrossEntropy::Sigmoid_CrossEntropy(lint axis)
    :
    op::Function {std::string {"Sigmoid_CrossEntropy"}, true},
    m_sc {std::make_unique<la::Sigmoid_CrossEntropy<float>>(axis)}
{
    this->m_output = std::make_shared<var::Tensor<float>> ();
}

Sigmoid_CrossEntropy::Sigmoid_CrossEntropy(const Sigmoid_CrossEntropy & other)
    :
    op::Function {other},
    m_sc {std::make_unique<la::Sigmoid_CrossEntropy<float>>(*(other.m_sc))}
{
    this->m_output = std::make_shared<var::Tensor<float>> ();
}

Sigmoid_CrossEntropy::Sigmoid_CrossEntropy(Sigmoid_CrossEntropy && other)
    :
    op::Function {other},
...
Sigmoid_CrossEntropy & Sigmoid_CrossEntropy::operator = (const Sigmoid_CrossEntropy & other)
{
    op::Function::operator = (other);
    this->m_sc = std::make_unique<la::Sigmoid_CrossEntropy<float>>(*(other.m_sc));

    return *this;
}

Sigmoid_CrossEntropy & Sigmoid_CrossEntropy::operator = (Sigmoid_CrossEntropy && other)
{
    op::Function::operator = (other);
...
void Sigmoid_CrossEntropy::set_inputs(const std::shared_ptr<op::Function> & self, 
                                    const std::vector<std::shared_ptr<op::Variable>> & inputs)
{
    if (inputs.size() != 2)
    {
        throw std::invalid_argument(std::string("Number of inputs for Sigmoid_CrossEntropy operation is not 2"));
    }

    op::Function::set_inputs(self, inputs);
}

void Sigmoid_CrossEntropy::forward()
{
...
void Sigmoid_CrossEntropy::backward()
{
    var::Tensor<float> *output_ptr = dynamic_cast<var::Tensor<float>*>(this->m_output.get());
...
void Sigmoid_CrossEntropy::clear_cache()
{
