...
/******************************************************************************
 * "MaxPool" is a function which does forward-propagation and back-propagation
 * of the following operation:
 * 
 *     MaxPool ( tensor_a )
 * 
 ******************************************************************************/
class MaxPool : public op::Function
{
public:
    ...
    MaxPool(lint pad_h, lint pad_w, lint kernel_h, lint kernel_w, lint stride_h, lint stride_w);

    // Copy constructor
    MaxPool(const MaxPool & other);

    // Move constructor
    MaxPool(MaxPool && other);

    // Copy assignment
    MaxPool & operator = (const MaxPool & other);

    // Move assignment
    MaxPool & operator = (MaxPool && other);
