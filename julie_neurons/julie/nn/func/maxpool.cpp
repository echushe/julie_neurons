...
MaxPool::MaxPool(lint pad_h, lint pad_w, lint kernel_h, lint kernel_w, lint stride_h, lint stride_w)
    :
    op::Function {std::string {"MaxPool"}, false},
    m_pad_h {pad_h},
    m_pad_w {pad_w},
    m_kernel_h {kernel_h},
    m_kernel_w {kernel_w},
    ...
    , m_onednn_pool2d {std::make_unique<julie::la::Pool2dOneDNN<float>>(
        pad_h, pad_w, kernel_h, kernel_w, stride_h, stride_w, julie::la::Pool2dOneDNN<float>::PoolType::Max)}
#endif
{
    this->m_output = std::make_shared<var::Tensor<float>> ();
}

MaxPool::MaxPool(const MaxPool & other)
    :
    op::Function {other},
...
MaxPool & MaxPool::operator = (const MaxPool & other)
{
...
MaxPool & MaxPool::operator = (MaxPool && other)
{
    op::Function::operator = (other);
...
void MaxPool::set_inputs(const std::shared_ptr<op::Function> & self, 
                                    const std::vector<std::shared_ptr<op::Variable>> & inputs)
{
    if (inputs.size() != 1)
    {
        throw std::invalid_argument(std::string("Number of inputs for MaxPool operation is not 1"));
    }
