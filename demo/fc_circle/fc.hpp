 ...
 * neural network applying a cycle from layer-2's output to layer-2's input.
 * 
...
void fc_train_mnist(const std::string &dataset_dir)
{
    ...
    julie::la::iMatrix<float> B_1_mat { 0, julie::la::Shape {200}};
    W_1_mat.gaussian_random(0, 0.1);
    
    // Initialize weights of the second layer
    julie::la::iMatrix<float> W_2_mat { julie::la::Shape {200, 200}};
    julie::la::iMatrix<float> B_2_mat { 0, julie::la::Shape {200}};
    W_2_mat.gaussian_random(0, 0.1);

    // Initialize weights of the third layer
    julie::la::iMatrix<float> W_3_mat { julie::la::Shape {200, 200}};
    julie::la::iMatrix<float> B_3_mat { 0, julie::la::Shape {200}};
    W_3_mat.gaussian_random(0, 0.1);

    // Initialize weights of the fourth layer
    julie::la::iMatrix<float> W_4_mat { julie::la::Shape {200, 10}};
    ...
    auto act_4 = std::make_shared<julie::nn::func::SoftMax> (1);

    // Add function nodes with their inputs and outputs to the graph
    the_model_graph.add_node(matmul_4, {act_3->get_output(), W_4});
    the_model_graph.add_node(add_4, {matmul_4->get_output(), B_4});
    the_model_graph.add_node(act_4, {add_4->get_output()});

    // --------------------------------------------------------------------//
    //   Add circle from the second layer output to the second layer input //
    // --------------------------------------------------------------------//

    auto add_1_2 = std::make_shared<julie::nn::func::Add> ();
    
    the_model_graph.add_node(add_1_2, {act_1->get_output(), act_2->get_output()});
    the_model_graph.add_node(matmul_2, {add_1_2->get_output(), W_2});
    ...
    for (lint itr = 0; itr < 400; ++itr)
    {
        ...
        the_model_graph.forward(act_4->get_output());
        ...
        auto pred_batch = act_4_output->val()->argmax(1);
        ...
        if ((itr + 1)% 100 == 0)
        {
            act_2_output->val(julie::la::iMatrix<float>{0, julie::la::Shape {val_batch_size, 200}, mat_type});
            ...
            act_2_output->val(julie::la::iMatrix<float>{0, julie::la::Shape {train_batch_size, 200}, mat_type});
        }
    }
}
