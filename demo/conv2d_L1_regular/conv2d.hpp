 ...
 * neural network with L1 regulariztion using the MNIST dataset.
 * 
...
std::shared_ptr<julie::nn::var::Scalar<float>> L1_regular(
    julie::op::Graph &the_model_graph,
    const std::shared_ptr<julie::op::Function> &loss_func)
{
    std::shared_ptr<julie::op::Function> loss_sum = std::make_shared<julie::nn::func::Sum> ();
    the_model_graph.add_node(loss_sum, {loss_func->get_output()});

    auto trainables = the_model_graph.all_trainable_variables();
    for (auto trainable : trainables)
    {
        auto p_abs = std::make_shared<julie::nn::func::Abs> ();
        auto p_scale = std::make_shared<julie::nn::func::Scale> ();
        auto p_sum = std::make_shared<julie::nn::func::Sum> ();
        auto scalar_add = std::make_shared<julie::nn::func::ScalarAdd> ();
        scalar_add->to_loss_function();
        
        auto weight_decay_rate = std::make_shared<julie::nn::var::Scalar<float>> (0.1);

        the_model_graph.add_node(p_abs, {trainable});
        the_model_graph.add_node(p_scale, {p_abs->get_output(), weight_decay_rate});
        the_model_graph.add_node(p_sum, {p_scale->get_output()});
        the_model_graph.add_node(scalar_add, {loss_sum->get_output(), p_sum->get_output()});
        
        loss_sum = scalar_add;
    }

    return std::dynamic_pointer_cast<julie::nn::var::Scalar<float>> (loss_sum->get_output());
}
...
void conv2d_mnist(const std::string &dataset_dir)
{
    ...
    julie::la::iMatrix<float> W_2_mat {julie::la::Shape{32, 16, 3, 3}};
    ...
    B_1->trainable(true);
    auto A_1 = std::make_shared<julie::nn::var::Scalar<float>> (0.0);
    A_1->trainable(true);
    ...
    auto maxpool_1 = std::make_shared<julie::nn::func::MaxPool> (0, 0, 4, 4, 2, 2);
    the_model_graph.add_node(maxpool_1, {act_1->get_output()});
    ...
    B_2->trainable(true);
    auto A_2 = std::make_shared<julie::nn::var::Scalar<float>> (0.0);
    A_2->trainable(true);

    auto conv_2 = std::make_shared<julie::nn::func::Conv2d> (1, 1, 1, 1);
    auto act_2 = std::make_shared<julie::nn::func::ReLU> ();

    the_model_graph.add_node(conv_2, {maxpool_1->get_output(), W_2, B_2});
    ...
    the_model_graph.add_node(maxpool_2, {act_2->get_output()});
    ...
    B_3->trainable(true);
    auto A_3 = std::make_shared<julie::nn::var::Scalar<float>> (0.0);
    A_3->trainable(true);

    auto conv_3 = std::make_shared<julie::nn::func::Conv2d> (1, 1, 1, 1);
    auto act_3 = std::make_shared<julie::nn::func::ReLU> ();

    the_model_graph.add_node(conv_3, {maxpool_2->get_output(), W_3, B_3});
    the_model_graph.add_node(act_3, {conv_3->get_output()});

    // Construct the third pooling layer
    ////////////////////////////////////////////////////////////////////////////////////

    auto maxpool_3 = std::make_shared<julie::nn::func::MaxPool> (0, 0, 2, 2, 2, 2);
    the_model_graph.add_node(maxpool_3, {act_3->get_output()});
    ...
    B_4->trainable(true);
    auto A_4 = std::make_shared<julie::nn::var::Scalar<float>> (0.0);
    A_4->trainable(true);

    auto conv_4 = std::make_shared<julie::nn::func::Conv2d> (0, 0, 3, 3);
    auto act_4 = std::make_shared<julie::nn::func::ReLU> ();

    the_model_graph.add_node(conv_4, {maxpool_3->get_output(), W_4, B_4});
    ...
    std::shared_ptr<julie::op::Function> loss_func = std::make_shared<julie::nn::func::SoftMax_CrossEntropy> (1);
    ...
    auto loss_output = L1_regular(the_model_graph, loss_func);
    loss_func = loss_output->get_provider();

    the_model_graph.add_input(x);
    the_model_graph.set_device(mat_type);

    std::cout << the_model_graph.to_string() << std::endl;
    std::cout << "Finish printing the graph" << std::endl;
